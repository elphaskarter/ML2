...
def plot(data, clsfd_img, veg_classes):
    veg_mask = (clsfd_img == 1)
    masked_veg = np.ma.masked_where(~veg_mask, clsfd_img)
    # gt_bnry = np.isin(gt_labels, veg_classes).astype(int)
    ...
    ax[1].imshow(gt_labels, cmap='jet')
    ax[1].set_title('Ground Truth', fontsize=8)
    ax[1].axis('on')

    ax[2].imshow(clsfd_img, cmap='jet')
...
# # Preprocess non-background pixels
# X_full_slctd = X[:, idx]
# X_full_slctd = scaler.transform(X_full_slctd)
# y_full_pred = model.predict(X_full_slctd)

# # reshape prediction to original grnd truth data shape
# full_pred_img = np.zeros(gt_labels.shape, dtype=int)
# mask = gt_labels != 0
# full_pred_img[mask] = y_full_pred

# # Optionally remap the binary predictions (1 becomes 2 for vegetation)
# full_pred_img_remap = full_pred_img.copy()
# full_pred_img_remap[full_pred_img == 1]

# # Plot the full image
# plot(gt_labels, full_pred_img_remap, [2, 4])
...
# Remap '1' to '2' for vegetation
full_pred_img_remap = full_pred_img.copy()
full_pred_img_remap[full_pred_img_remap == 1] = 2
plot(rgb_image, full_pred_img_remap, [2, 4])
...
# y_opt_pred  = (y_test_probs >= best_threshold).astype(int)
